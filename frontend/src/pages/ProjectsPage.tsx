          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              >
                {selectedAgent ? (
                  <>
                    <span className="text-xl flex-shrink-0">{selectedAgent.ui.icon}</span>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-gray-900 dark:text-white truncate">{selectedAgent.name}</div>
                      {selectedAgent.description && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{selectedAgent.description}</div>
                      )}
                    </div>
                  </>
                ) : (
                  <span className="text-sm text-gray-400">{t('projects.form.agentType')}</span>
                )}
                    ...
                    {agents.filter(agent => agent.enabled).map((agent) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors first:rounded-t-lg last:rounded-b-lg ${
                          formData.agentId === agent.id ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                        }`}
                      >
